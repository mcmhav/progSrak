...
local
   ...
   fun {DefProc Tokens Rest}
	 ...
	 Tokens = '{' | '$' | Rest1
         'proc_val'( {IdentList Rest1 '}'|Rest2}
                    {Stat Rest2 'end'|Rest} )
      end
   end
   ...
   fun {Feature Tokens Rest}
      local F Rest1 Rest2 in 
         ...
         [] 'true' | R then Rest1 = R F = recpat('true' nil)
         [] 'false' | R then Rest1 = R F = recpat('false' nil)
   ...
   fun {If Tokens Rest}
      ...
      try
         local Rest1 Rest2 in
            'condit_stat'( {Ident Tokens 'then'|Rest1}
                    {Stat Rest1 'else'|Rest2}
                    {Stat Rest2 'end'|Rest} )
         end
      catch E then raise ifPE(E) end
      end
   end
   ...
   fun {Case Tokens Rest}
      ...
      try
         local Rest1 Rest2 Rest3 in
            'patter_match'( {Ident Tokens 'of'|Rest1}
